...
Seven views: the calculator with its reasoning panel, scientific mode, a mixing
simulator, a state choropleth, year-over-year trends, chicken facts, and a
sources table. Every figure carries a colour-coded confidence badge — green measured, blue study,
...
Of the 21 loss factors in the model, **11 are unsourced estimates (52%)**. Only 8 of those affect the **count** answer; the other 3 are mass-only and cannot move it. That distinction is tracked and reported by the audit rather than glossed over.

Corpus: **45 sources**, 46 facts, 5 products across 3 active species, 30 tables. Every statistic is cited and the build fails if one is not.
